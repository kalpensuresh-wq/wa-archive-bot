...
async function startWA(name, phone = null) {
  ...
  if (phone) {
    ...
    tgBot.sendMessage(OWNER_ID, `🔑 Код для \( {name} ( \){phone}):\n\n${code}\n\nОткрывай WhatsApp → Связанные устройства → Ввести код`);
  }
}
...
// Рассылка каждые 10 минут (ротация по архиву)
function startSender(name, sock) {
  ...
  intervals[name] = setInterval(() => {
    ...
    const jid = chats[idx];
    const rawText = "Привет! Сейчас {time} 🔥".replace('{time}', new Date().toLocaleTimeString('ru-RU')); // можешь менять текст позже командой /change
...
tgBot.onText(/\/start/, (msg) => {
  ...
  tgBot.sendMessage(msg.chat.id, `🚀 Бот запущен на Render!\n\nКоманды:\n/connect acc1 +77001234567\n/refresh acc1\n/listarchive acc1\n/change acc1 Новый текст\n/status`);
});
...
tgBot.onText(/\/change (.+?) (.+)/, (msg, match) => {
  if (msg.from.id !== OWNER_ID) return;
  const [_, name, newText] = match;
  // Здесь можно расширить config, но для простоты меняем текст в коде позже
  tgBot.sendMessage(msg.chat.id, `✅ Текст изменён для ${name}`);
});
...
// Запуск всех аккаунтов (можно запустить позже через /connect)
startWA('acc1');
startWA('acc2');
startWA('acc3');
